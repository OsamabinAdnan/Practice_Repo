...
//Type Alias
// the advantage over anonymous type is that we give name in type Alias unlike Anonymous type so we can reuse it again and again
type Teacher = {
    name:string,
    exper: number | string
}
...
let teach:Teacher = { //we can use Teacher alias where we want to unlink anonymous type
    name: "Ahmed",
    exper: "5 years"
}

// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Interface >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//In TypeScript, both type aliases and interfaces are used to define object shapes and types.
...
type Sphere = { //You can use either interface or type Alias, properties name should be matched, doesnt matter what are the name of interface or type Alias
    diameter:number,
    radius: number,
}
...
let x: {
    id: number,
    [x:string]:any //A type can include an index signature to explicitly indicate that excess properties are permitted in with fresh objects:
}
...
var A: { id: number, [x: string]: any };//Note now 'x' can have any name, just that the property should be of type string
...
type EmployeeDetail = personDetail & officialDetail
